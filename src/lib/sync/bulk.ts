// ============================================================
// Bulk GHL → Supabase reconcile, treating GHL as the source of truth:
//   1. Import/update every GHL contact holding an access tag
//      (creating auth users for new contacts).
//   2. Reconcile existing members GHL no longer tags — a member whose access
//      tag was removed (or whose contact was deleted) in GHL is deactivated,
//      so a cancellation propagates.
//
...
import { syncMember, refreshMembersFromGhl } from './index'

export interface BulkGhlSyncResult {
  total: number
  synced: number
  skipped: number
  deactivated: number
...
export async function runBulkGhlSync(
  opts: { requestId?: string } = {}
): Promise<BulkGhlSyncResult> {
  ...
  log.info('Bulk sync complete', {
    metadata: { total: contacts.length, synced, skipped, deactivated, errors: errors.length },
  })

  return { total: contacts.length, synced, skipped, deactivated, errors }
}
