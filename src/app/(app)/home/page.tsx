...
import type { Booking, Announcement, Promotion, MemberWithProfile } from '@/types'
...
export default function HomePage() {
  ...
  async function loadHomeData() {
    ...
    const [bookingRes, announcementRes, promoRes, memberRes] = await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="hero-banner">
        <p className="text-xs uppercase tracking-widest text-white/40 mb-1">{greeting}</p>
        ...
        {loading ? (
          ...
          <Link href="/book" className="block mt-4 rounded-xl bg-white/[0.06] border border-gold/20 p-3.5 flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-gold/15 flex items-center justify-center text-gold text-xl flex-shrink-0">
              ⛳
            </div>
            <div>
              <p className="text-xs text-white/40 uppercase tracking-wider mb-0.5">Your next round</p>
              <p className="text-sm text-white font-medium">
                {formatBookingDate(nextBooking.booking_date)} · {formatTeeTime(nextBooking.tee_time)}
              </p>
              <p className="text-xs text-white/40 mt-0.5">
                Park Hyatt Aviara{nextBooking.guest_name ? ` · With ${nextBooking.guest_name}` : ''}
              </p>
            </div>
          </Link>
        ) : (
          <Link href="/book" className="block mt-4 rounded-xl bg-white/[0.06] border border-white/10 p-3.5 flex items-center gap-3">
      ...
      <div className="px-5 pt-5 space-y-6 pb-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/more/announcements" className="text-xs text-green-600">See all →</Link>
          </div>
          {loading ? (
            ...
            <p className="text-sm text-green-900/40 italic">No announcements yet.</p>
          )}
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/members" className="text-xs text-green-600">All members →</Link>
          </div>
          {loading ? (
            <div className="card">
              <MemberRowSkeleton />
            ...
            <div className="card">
              {newMembers.map(m => (
                <Link key={m.id} href={`/members/${m.id}`} className="member-row block">
            ...
            <p className="text-sm text-green-900/40 italic">No members to spotlight yet.</p>
          )}
        </section>
        ...
        {(loading || promotions.length > 0) && (
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/more/promotions" className="text-xs text-green-600">View all →</Link>
            </div>
            {loading ? (
              <div className="promo-card p-4"><CardSkeleton lines={3} /></div>
            ) : (
...
function AnnouncementCard({ announcement }: { announcement: Announcement }) {
  ...
  return (
    <div className="card card-pad flex gap-3 items-start">
      <span className="text-xl mt-0.5 flex-shrink-0">{icons[announcement.type] ?? '📌'}</span>
      <div>
        <p className="text-sm font-medium text-green-900">{announcement.title}</p>
