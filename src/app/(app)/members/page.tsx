    ...
    <div>
      ...
      <div className="px-5 pt-4">
        <div className="flex items-center gap-2 bg-green-50 border border-green-900/10 rounded-xl px-3.5 py-2.5">
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="text-green-900/30 text-lg leading-none">×</button>
          )}
        </div>
      </div>
...
function MemberRow({ member: m, currentUserId }: { member: MemberWithProfile; currentUserId?: string }) {
  const isMe = m.id === currentUserId

  return (
    <Link href={isMe ? '/more/profile' : `/members/${m.id}`} className="member-row flex items-center gap-3">
      <Avatar
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {isMe && (
            <span className="text-xs text-gold bg-green-900/10 px-2 py-0.5 rounded-full">You</span>
          )}
        </div>
...
function SearchIcon() {
  return (
    <svg className="w-4 h-4 text-green-900/35 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
