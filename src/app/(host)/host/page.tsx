  ...
  return (
    <div className="p-4 sm:p-8 max-w-4xl mx-auto">
      <AdminPageHeader
        ...
        description={
          data ? `Hosting as ${data.host.name}` : "Your events and credits"
        }
        action={
          <Link href="/host/events" className="btn btn-gold btn-sm">
            Manage events
          </Link>
        }
      />
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
            <StatCard
              label="Earned"
              value={fmtMoney(data.stats.credits.earned)}
              sub="Approved to date"
              colour="green"
            />
            <StatCard
              label="Redeemed"
              value={fmtMoney(data.stats.credits.redeemed)}
              sub="Spent"
              colour="gray"
            />
            <StatCard
              label="Available"
              value={fmtMoney(data.stats.credits.balance)}
              sub="Balance"
              colour="gold"
              large
            />
